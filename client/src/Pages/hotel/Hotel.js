...
const Hotel = () => {
  ...
  const {data, loading, error} = useFetch(`/api/hotels/get/${id}`)
  ...
  const days = (dates[0]) ? (dayDifference(dates[0].endDate, dates[0].startDate)) : null;
  ...
  const handleClick = () => {
    if(user) {
      ...
      // Save the current location to return to this page after login
